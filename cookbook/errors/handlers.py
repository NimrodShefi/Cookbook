...
def error_OperationalError(error):
    return redirect("errors/error.html", 
                            error = True,
                            error_type="Something went wrong (500)", 
                            error_msg="We're experiencing some trouble on our end. Please try again in the near future"), OperationalError
...
def error_IntegrityError(error):
    return render_template("errors/error.html",
                    error = True, 
                    error_type="Something went wrong (500)", 
                    error_msg="We're experiencing some trouble on our end. Please try again in the near future"), IntegrityError
...
def error_PendingRollbackError(error):
    return render_template("errors/error.html",
                    error = True,
                    error_type="Something went wrong (500)", 
                    error_msg="We're experiencing some trouble on our end. Please try again in the near future"), PendingRollbackError
